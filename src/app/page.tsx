...
const page = () => {
  ...
  useEffect(() => {
    ...
    if(leaderboard.length === 0) {
      axios.get(`${baseURL}/api`).then(data => setLeaderboard(data.data))
    }
  ...
  const updateLeaderBoard = (teamId:number, teamName:string) => {

    if(!gameStarted) {
      alert('PPress ⏺️🤾‍♂️ to start !!');
      return;
    }
  ...
  return (
    <div className="bg-slate-200">
      <nav className="text-center font-bold w-full h-16 text-3xl pt-3 bg-slate-400 text-indigo-900 flex items-center justify-evenly">
          ...
          <div className="" onClick={() => setGameStarted(true)}><StartButton socket = {socket}/></div>
        </nav>
